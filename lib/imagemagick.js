...
ImageMagick = (function() {
  ...
  ImageMagick.prototype.composite = function(options, callback) {
    ...
    return this.exec(command, function(error, stdout, stderr) {
      var compose;
      if (error || stderr) {
        if (!/convert: unable to access configure file.+@ warning/.test(error || stderr)) {
          throw "Error in creating canvas (" + filepath + "): " + (error || stderr);
        } else {
          console.log("  Warning in creating canvas (" + filepath + "): " + (error || stderr));
        }
      }
  ...
  ImageMagick.prototype.composeImage = function(filepath, image, downsampling, callback) {
    ...
    return exec(command, function(error, stdout, stderr) {
      if (error || stderr) {
        if (!/composite: unable to access configure file.+@ warning/.test(error || stderr)) {
          throw "Error in composite (" + filepath + "): " + (error || stderr);
        } else {
          console.log("  Warning in composite (" + filepath + "): " + (error || stderr));
        }
      }
      return callback();
    });
